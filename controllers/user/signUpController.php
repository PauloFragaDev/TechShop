...
$_SESSION['errorsSignUp'] = array();

if (empty($_POST['email'])) {
    $_SESSION['errorsSignUp']['emailEmpty'] = "El campo Email esta vacio.";
}
if (empty($_POST['fullname'])) {
    $_SESSION['errorsSignUp']['passEmpty'] = "El campo Password esta vacio.";
}
if (empty($_POST['password'])) {
    $_SESSION['errorsSignUp']['fullNameEmpty'] = "El campo Full Name esta vacio.";
}

if (empty($_FILES['image']['name'])){
    $_SESSION['errorsSignUp']['imageEmpty'] = "No ha seleccionado una imagen.";
}

if (!empty(UserTechService::getUserByEmail($email))){
    $_SESSION['errorsSignUp']['emailDuplicate'] = "El Email esta en uso.";
}

if ($_FILES['image']['type'] != "image/png"){
    $_SESSION['errorsSignUp']['extensioImg'] = "La extension no es png.";
}

if (count($_SESSION['errorsSignUp'])>0) {
    header('Location: ../../views/signup.php');
    exit();
}
